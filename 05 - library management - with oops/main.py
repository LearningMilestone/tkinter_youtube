...
class App(tkinter.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Library Management App")
    ...
    def search(self):
        ...
        self.bktitle = self.ent_bktitle.get()
        self.bkauthor = self.ent_bkauthor.get()
        self.bkisbn = self.ent_bkISBN.get()
        self.bkrating = self.rating_option.get()

        with open('books-data.csv', 'r', encoding='utf-8-sig') as f:
            dict_reader = csv.DictReader(f)

            if len(self.bktitle) == 0 and len(self.bkauthor) == 0 and len(self.bkisbn) == 0 and self.bkrating == "Select Rating":
                ...
                for row in dict_reader:
                    print(self.bktitle.title())
                    print(self.bkauthor.title())
                    print(row['title'])
                    print(row['authors'])
                    if self.bktitle.title() in row['title'] and self.bkauthor.title() in row['authors'] and self.bkisbn.title() in \
                            row['isbn'] and float(row['average_rating'])> int(self.bkrating.title()[0]):
                        self.book_list.insert(END, row)
                        print('here')

    def export(self):
        # open filedialogue and save file in csv format
        self.save_name = filedialog.asksaveasfilename(initialdir='/',
                                                 title="Save File",
                                                 filetypes=[("csv file(*.csv)", "*.csv"), ('All Files', '*.*')],
                                                 defaultextension=[('All tyes(*.*)', '*.*'),
                                                                   ("csv file(*.csv)", "*.csv")])

        with open(self.save_name, "w", newline='', encoding='utf-8-sig') as f:
            writer_csv = csv.writer(f)
            writer_csv.writerow(['Title', 'Author', "ISBN", "Rating"])
            self.listbox_data = self.book_list.get(0, END)
            for data in self.listbox_data:
                data = eval(data)
